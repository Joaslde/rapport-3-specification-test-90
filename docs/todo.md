...
| **📬 Formulaires du site vitrine** | ✅ **25 août 2026** — le back-office `/admin` accueille trois nouvelles rubriques alimentées par cabinetlequart.com : **Liste VIGIE** (inscriptions), **Messages** (contact, vue de type messagerie avec panneau de lecture défilant) et **Candidatures** (dossiers d'admission, avec décision Retenir / Refuser). Tables `vigie_liste_attente`, `contacts` et `candidatures`, mêmes règles RLS que `resultats` : `anon` insère, ne lit jamais. ⚠️ `candidatures` contient des données financières — la liste ne renvoie pas les montants, seul le détail d'un dossier les expose. Fonction `admin-resultats` v4. |
...
| 26 août 2026 | **Logo officiel du cabinet reçu, remplace le monogramme « 90 ».** `public/logo-favicon.png` fourni par l'utilisateur (logo Le Quart : compas + inscription « Conseil privé de dirigeants »). Le cercle Laiton avec « 90 » en `EnteteCabinet.vue` est retiré au profit de ce logo. Ancien favicon (`favicon.svg`, `favicon.ico`, template Vue) supprimé et remplacé par le même fichier PNG dans `index.html`. **Vérifié :** build sans erreur, logo présent dans `dist/`, 222 tests toujours au vert, lint propre, audit du bundle sans fuite. |
