...
| **📬 Formulaires du site vitrine** | ✅ **25 août 2026** — le back-office `/admin` accueille deux nouvelles rubriques alimentées par cabinetlequart.com : **Liste VIGIE** (inscriptions) et **Messages** (formulaire de contact, vue de type messagerie avec panneau de lecture défilant). Tables `vigie_liste_attente` et `contacts`, mêmes règles RLS que `resultats` : `anon` insère, ne lit jamais. Fonction `admin-resultats` v3. |
